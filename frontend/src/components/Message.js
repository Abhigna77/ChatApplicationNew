...
const Message = ({typingMessage,message,currentfriend,scrollRef}) => {
  ...
  return (
     ...
     index===message.length-1 && m.senderId===myInfo.id?m.status==='seen'?
     <img className='img' src={`./image/${currentfriend.image}`} alt='' />:
     m.status==='delivered' ?<span><FaRegCheckCircle /></span>:
     <span><FaRegCheckCircle /></span>:''
          ...
          <p className='message-text'>{m.message.text===''?<img src={`./image/${m.message.image}`} />:m.message.text} </p>
                   ...
                   {moment(m.createdAt).startOf('mini').fromNow()}              
                       ...
                       <img src={`./image/${currentfriend.image}`} alt='' />
                       <h3>Say Hi to {currentfriend.userName} </h3>
