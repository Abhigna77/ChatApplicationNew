...
import { useDispatch, useSelector } from "react-redux"; // useSelector is used to extract data from redux store
import {
  getFriends,
  messageSend,
  getMessage,
...
import { NEW_USER_ADD_CLEAR } from "../store/types/messengerType";
...
const Messenger = () => {
  ...
  useEffect(() => {
    socket.current = io("ws://localhost:8000"); //When you use the useRef hook, it returns an object with a current property.
    ...
    socket.current.on("msgSeenResponse", (msg) => { //.on is to set up an event listener
      dispatch({
        type: "SEEN_MESSAGE",// dispatching a redux action,type of action
        payload: { // contains data related to action
          msgInfo: msg,
        },
      });
    });
  ...
  useEffect(() => {
    socket.current.emit("addUser", myInfo.id, myInfo); // browser to server
  }, []);
  useEffect(() => {
  ...
  const inputHandle = (e) => { // when message is being typed
    setNewMessage(e.target.value);  // line 39
  ...
  const sendMessage = (e) => {  // when the message is sent
    e.preventDefault();
    sendingSPlay();
    const data = {
  ...
  useEffect(() => {
    scrollRef.current?.scrollIntoView({ behavior: "smooth" }); // the scrolling animation is smooth
  }, [message]);
