...
const FriendList = (props) => {
  ...
  return (
     ...
     {
          msgInfo && msgInfo.message.text ? <span className={ msgInfo?.senderId !== myId && msgInfo?.status !== undefined && msgInfo.status !== 'seen'?'unseen_message ' : '' }>{msgInfo.message.text.slice(0, 10)+'_' }</span> : msgInfo && msgInfo.message.image ? <span>sent an image </span> : <span> joined: </span>
     }
                 ...
                 </div>
     {
          ...
          <div className='seen-unseen-icon'>
               {
                    msgInfo.status === 'seen' ?
                    <img src={`./image/${fndInfo.image}`} alt='' /> : msgInfo.status === 'delivered' ? <div className='delivered'> <FaRegCheckCircle /> </div> : <div className='unseen'> </div>
               }
